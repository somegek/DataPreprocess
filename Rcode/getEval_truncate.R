getEval_truncate <- function(DT_FCT){
  ...
  # calculated mean of error squared
  ...
  DT_RES[, (errSumAbsList) := lapply(.SD, function(x) mean(x)), .SDcols = errAbsList, by = c('FCT_TOPIC', 'FCT_HORIZON')]
  ...
  DT_RES[, (errAbsList) := NULL]
  DT_RES[, (errSumAbsList) := NULL]
