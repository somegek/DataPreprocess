...
for (corr in c(0.5,0.6,0.7,0.8)){
  ...
  for(epsVar in epsList){
    ...
    truncation <- c(-Inf,seq(from = -5, by = 1, to = 0))
    # > truncation
    ...
    # if(epsVar == 0.4){
    #   plot(fcstmse, type = 'l',ylim=c(0,0.4), ylab = 'MSPE', xlab='Threshold',xaxt = "n", main = paste0('Correlation: ',corr))
    # }else if(epsVar == 0.5){
    #   lines(fcstmse, type = 'l', col='red')
    # }else if(epsVar == 0.6){
    #   lines(fcstmse, type = 'l', col='orange')
    # }else if(epsVar == 0.7){
    #   lines(fcstmse, type = 'l', col='blue')
    # }else if(epsVar == 0.8){
    #   lines(fcstmse, type = 'l', col='forestgreen')
    # }else if(epsVar == 0.9){
    #   lines(fcstmse, type = 'l', col='purple')
    # }
  }
  # legend('topright',as.character(epsList),
  #        fill=c("black","red","orange","blue","forestgreen","purple")
  # )
}
}
DT <- unique(DT[,(paste0("V",1:7)):=lapply(.SD,mean), .SDcols = paste0("V",1:7), by = c('correlation','errorVar')])
for (corr in c(0.5,0.6,0.7,0.8)){
